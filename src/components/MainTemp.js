...
export default function MainTemp(props){
    ...
    const locationDivs = locations.map(el=>{
        return <div key={nanoid()} id={el.id} onClick={props.divClick} className='cursor-pointer border h-10 bg-white border-black flex items-center justify-center'>{el.name}, {el.admin1}{el.admin1 !== undefined ? ",": el.admin1} {el.country_code}</div>
    })
    ...
    return(
            <div  className="px-8 pb-8">
                ...
                <div  style={props.colorDarkMode}
                    className='text-3xl flex justify-center pt-8 font-mono text-center font-bold'>{props.judOras[0].oras}{props.judOras[0].jud !== undefined ? "," : props.judOras[0].jud} {props.judOras[0].jud}</div>
                <div style={props.colorDarkMode}
